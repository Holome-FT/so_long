...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/07/30 22:37:34 by raamayri          #+#    #+#             */
/*   Updated: 2025/08/01 20:02:02 by raamayri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_calc_pinky_default_target(long long trg_xy[2], t_map *map)
{
	t_player	*player;
...
static void	ft_calc_pinky_trg(long long trg_xy[2], t_map *map, t_ghost *ghost)
{
	if (ghost->state == GHOST_DEFAULT)
		ft_calc_pinky_default_target(trg_xy, map);
	...
	long long new_xy[4])
{
	...
	trg_xy[0] = new_xy[0];
	trg_xy[1] = new_xy[1];
	ft_calc_pinky_trg(trg_xy, map, ghost);
